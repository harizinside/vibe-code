...
export function setupOpenAPI(app: Hono) {
  const openapi = fromHono(app, {
    schema: {
      ...
      security: [{ BearerAuth: [] }, { ApiKeyAuth: [] }, { OAuth2: ["read:users", "read:products"] }],
      servers: [
        { url: "https://api.global-economy.com/v1", description: "Production server" },
        { url: "https://staging-api.global-economy.com/v1", description: "Staging server" },
      ],
      tags: [
        { name: "User", description: "Operations related to user management" },
        { name: "Product", description: "Operations related to product management" },
      ],
    },
    openapi_url: "/doc",
    docs_url: "/ui",
